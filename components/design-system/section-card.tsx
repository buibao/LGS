...
export function SectionCard({
  ...
  return (
    <Card className={cn("overflow-hidden rounded-[28px] border border-border/70 bg-white/96 shadow-[0_22px_50px_-38px_rgba(15,23,42,0.18)]", className)}>
      {hasHeader ? (
        <CardHeader className="flex flex-col gap-4 border-b border-border/70 bg-[linear-gradient(180deg,rgba(249,251,255,0.95),rgba(255,255,255,0.92))] px-5 py-5 sm:flex-row sm:items-start sm:justify-between md:px-6">
          <div className="space-y-2">
            {title ? <CardTitle className="text-[1.05rem] font-semibold text-slate-950">{title}</CardTitle> : null}
            {description ? <CardDescription className="max-w-2xl text-[0.92rem] leading-6 text-slate-600">{description}</CardDescription> : null}
          </div>
